#let metadata = toml("./template/info.toml")
...
#let info-block(
  ...
  table(
    columns: if use-photo {(1fr, 1fr)} 
                else {(1fr, 1fr, 1fr)},
    stroke: none,
    ..info.pairs().map(((key, val)) => info-block-style(icons.at(key), info-value(val), color, include_icons))
  )
}
...
#let reference-entry(
  ...
  company: "Company",
  telephone: "Telephone",
  email: (link: "mailto:Email",label:"Email"),
) = {
  table(
    columns: (3fr, 2fr),
    inset: 0pt,
    ...
    [],
    table.cell(colspan: 2)[#company-name-style(company)],
...
#let skill-entry(
  ...
  table(
    ...
    column-gutter: 3mm,
    align: left,
    ..skills.map(skill => skill-style(skill) )
  ) 
}
